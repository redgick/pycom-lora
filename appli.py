...
def setColorMsg(r, v, b, m1=None, m2=None):
    pycom.rgbled((r>>3)<<16|(v>>3)<<8|(b>>3))
    d.blinkLed()
    d.color(r,v,b)
    if m1:
        print ("m1=", m1)
...
while not lora.has_joined():
    time.sleep(2.5)
    if even:
        setColorMsg(0xFF,0xFF,0xFF,"Connecting to")
    else:
        setColorMsg(0x80, 0x80, 0x80, m2="Acklio's Network")
    even = not even
...
while not transOK:
    s.setblocking(True)
    s.settimeout(10)
    setColorMsg(0xFF, 0x00, 0xBB, m2="configuring...")
    try:
...
while True:
    ...
    if atqa != 0:
        ...
        if uid_len > 0:
            ...
            counter += 1
            setColorMsg(0xFF,0x8C,0, "Sending", "transaction")
            buz(440)  # Tag read
            print(
                "%d\tUID [%d]: %s" % (counter, uid_len, nfc.format_block(uid, uid_len))
            )
            d1 = cbor.dumps(uid[0:uid_len])
            c = cbor.dumps([counter, d1])
            print(c)
            ...
            while cptRetrans < 5:
                ...
                try:
                    ...
                    r = cbor.loads(data)
                    print(r)
                    setColorMsg(0x00, 0xFF, 0x00, r[1])
                    time.sleep(10)
        ...
        setColorMsg(0, 0, RGB_BLUE, "Swipe your badge", "here")
